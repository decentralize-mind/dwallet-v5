...
// Mock ERC-20 (DWT)
// ─────────────────────────────────────────────────────────────
async function deployMockDWT(owner) {
  const Token = await ethers.getContractFactory('MockERC20')
  const token = await Token.deploy('DWT Token', 'DWT', E18(10_000_000))
  return token
}
...
describe('Layer 4 — Staking & Rewards', function () {
  ...
  beforeEach(async () => {
    ...
    // ── StakingPool ──────────────────────────────────────────
    const SP = await ethers.getContractFactory('StakingPool')
    stakingPool = await SP.deploy(await dwt.getAddress(), owner.address)

    // ── DWTStaking ───────────────────────────────────────────
    const DS = await ethers.getContractFactory('DWTStaking')
    dwtStaking = await DS.deploy(await dwt.getAddress(), owner.address)

    // ── BoostedStaking ───────────────────────────────────────
    const BS = await ethers.getContractFactory('BoostedStaking')
...
// ─────────────────────────────────────────────────────────────
// Mock ERC-20 (deploy alongside tests)
// ─────────────────────────────────────────────────────────────
// Place MockERC20.sol in contracts/mocks/MockERC20.sol
