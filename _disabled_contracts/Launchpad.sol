...
contract Launchpad is ReentrancyGuard, Pausable, AccessControl, SecurityGated {
    ...
    // ── Timelock Escrow for Sale Proceeds ─────────────────────────────────────
    ...
    /// @dev idoId → sale proceeds in timelock escrow
    mapping(uint256 => SaleProceeds) public saleProceeds;
    
    /// @dev Withdrawal delay period (7 days)
    uint256 public constant WITHDRAWAL_DELAY = 7 days;
    
    // ── Events for Timelock ───────────────────────────────────────────────────
    event ProceedsLocked(uint256 indexed idoId, uint256 amount, uint256 unlockTime);
    event ProceedsWithdrawn(uint256 indexed idoId, uint256 amount);
    event WithdrawalVetoed(uint256 indexed idoId, address indexed by, uint256 newUnlockTime);
        ...
        withSignature(hash, signature)
    {
        ...
        // LOCK PROCEEDS IN TIMELOCK ESCROW (7-day delay)
        ...
        // Transfer raised funds to contract (held in escrow)
    ...
    // ── Timelock Withdrawal & Emergency Veto ───────────────────────────────────

    /**
     * @notice Withdraw IDO proceeds after timelock delay expires.
     *         Only callable by treasury/admin role.
     * @param idoId IDO to withdraw proceeds from
     */
    ...
    /**
     * @notice Emergency veto to delay withdrawal if suspicious activity detected.
     *         Only callable by Governor (multisig).
     * @param idoId IDO to veto withdrawal for
     * @param newUnlockTime New unlock timestamp (must be in future)
     */
    ...
    /**
     * @notice Re-enable withdrawal after emergency veto resolved.
     *         Only callable by Governor (multisig).
     * @param idoId IDO to re-enable withdrawal for
     */
