...
contract CrossChainMessenger is AccessControl, Pausable, ReentrancyGuard, SecurityGated {
    ...
    struct Message {
        ...
        uint256     expiresAt;      // @dev Added for explicit expiration tracking
     ...
     *      Validates that execution delay is less than message expiry.
     */
    function receiveMessage(
        ...
        // @dev Validate execution delay < message expiry to prevent stuck messages
     ...
     * @dev Gated by Protocol-wide pause via Layer 7.
     *      Checks expiration BEFORE execution delay to handle edge cases.
     */
    function executeMessage(bytes32 messageId) external nonReentrant whenNotPaused whenProtocolNotPaused {
        ...
        // @dev Check expiration FIRST to handle cases where delay > expiry
    ...
    function expireMessage(bytes32 messageId) external whenProtocolNotPaused {
        Message storage msg_ = messages[messageId];
        require(msg_.status == MessageStatus.Pending, "Messenger: not pending");
        require(block.timestamp > msg_.expiresAt, "Messenger: not expired yet");
