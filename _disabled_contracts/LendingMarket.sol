...
contract LendingMarket is ReentrancyGuard, Pausable, AccessControl, SecurityGated {
    ...
    uint256 public constant DEFAULT_LTV                 = 70e16;   // 70% default LTV
    uint256 public constant DEFAULT_LIQUIDATION_BONUS   = 5e16;    // 5% bonus for liquidators
    uint256 public constant DEFAULT_INTEREST_RATE_PER_BLOCK = 1e9; // ~2% APY at 12s blocks 
    ...
    uint256 public ltv                    = DEFAULT_LTV;
    uint256 public liquidationBonus       = DEFAULT_LIQUIDATION_BONUS;
    uint256 public interestRatePerBlock   = DEFAULT_INTEREST_RATE_PER_BLOCK;
    ...
    function setInterestRate(uint256 ratePerBlock, bytes32 hash, bytes calldata signature) 
        ...
        withSignature(hash, signature)
    {
        // @dev Validate against reasonable bounds to prevent predatory lending
    ...
    function _healthFactor(address borrower) internal view returns (uint256) {
        ...
        uint256 liqValue      = (collateralVal * LIQUIDATION_THRESHOLD) / PRECISION;
