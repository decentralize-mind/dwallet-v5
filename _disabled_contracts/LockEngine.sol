...
contract LockEngine is AccessControl, ReentrancyGuard {
     ...
     */
    function verifyAccess(address account, bytes32 role) external {
        _checkAccess(account, role);
    }
    
    function _checkAccess(address account, bytes32 role) internal {
        bool userHasRole = hasRole(role, account);
        ...
        emit AccessChecked(account, role, userHasRole || isSigner);
    }
     ...
     */
    function verifyTimeLock(address account, bytes32 actionId) external {
        _checkTimeLock(account, actionId);
    }
    ...
    function _checkTimeLock(address account, bytes32 actionId) internal {
     ...
     */
    function verifyState(bytes32 layerId) external {
        _checkState(layerId);
    }
    ...
    function _checkState(bytes32 layerId) internal {
    ...
    function _checkRateLimit(address account, bytes32 actionId, uint256 amount) internal {
    ...
    function _verifySignature(address expectedSigner, bytes32 hash, bytes calldata signature) internal {
        address recovered = ECDSA.recover(hash, signature);
