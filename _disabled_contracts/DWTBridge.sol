...
contract DWTBridge is AccessControl, ReentrancyGuard, Pausable, SecurityGated {
    ...
    uint256 public constant TRANSFER_EXPIRY = 7 days; // Transfer expires after 7 days
    ...
    struct PendingTransfer {
        ...
        uint256 amount;
        uint256 submittedAt;
        uint256 expiresAt;      // ← Added explicit expiration
        bool    executed;
     ...
     * @dev Gated by Protocol-wide pause via Layer 7. Checks expiration before execution.
     */
        ...
        external nonReentrant whenNotPaused whenProtocolNotPaused
    {
        ...
        if (block.timestamp > pt.expiresAt) {
            pt.executed = true; // Mark as executed to prevent future attempts
     ...
     */
    function expireTransfer(uint256 srcChainId, uint256 srcNonce) external whenProtocolNotPaused {
        bytes32 transferId = _transferId(srcChainId, srcNonce);
        PendingTransfer storage pt = _pendingTransfers[transferId];
