...
contract LiquidityIncentive is Ownable, ReentrancyGuard, SecurityGated {
     ...
     * @dev Gated by Layer 7 Protocol-wide pause and State Guard
     */
    function deposit(uint256 pid, uint256 amount) external nonReentrant whenProtocolNotPaused withStateGuard(LAYER_ID) {
        PoolInfo storage pool = poolInfo[pid];
     ...
     * @dev Gated by Layer 7 Protocol-wide pause and State Guard
     */
    function withdraw(uint256 pid, uint256 amount) external nonReentrant whenProtocolNotPaused withStateGuard(LAYER_ID) {
        PoolInfo storage pool = poolInfo[pid];
        UserInfo storage user = userInfo[pid][msg.sender];
     ...
     * @dev Gated by Layer 7 Protocol-wide pause
     */
    function harvest(uint256 pid) external nonReentrant whenProtocolNotPaused {
        updatePool(pid);
     ...
     * @dev Gated by Layer 7 Protocol-wide pause
     */
    function emergencyWithdraw(uint256 pid) external nonReentrant whenProtocolNotPaused {
        PoolInfo storage pool = poolInfo[pid];
        UserInfo storage user = userInfo[pid][msg.sender];
