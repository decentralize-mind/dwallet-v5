...
interface IVerificationEngine {
    function verifySignature(address account, bytes32 hash, bytes calldata signature) external;
    function verifySignatureWithNonce(address signer, uint256 nonce, bytes32 hash, bytes calldata signature) external;
...
interface ILockEngine {
    ...
    function securityController() external view returns (ISecurityController);
}
...
interface ISecurityController {
    ...
    function restoreCircuit() external;
