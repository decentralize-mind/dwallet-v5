...
struct ProposalCore {
    ...
    uint256 executeAfter;      // ← NEW: Earliest execution timestamp (timelock)
    uint256 forVotes;
...
// For satellite chains - simplified proposal structure
...
contract GovernanceHub is AccessControl, ReentrancyGuard, Pausable, SecurityGated {
    ...
    event ProposalTimelocked(uint256 indexed proposalId, uint256 executeAfter); // ← NEW
    ...
    uint256 public constant PROPOSAL_TIMELOCK = 48 hours; // ← NEW: Timelock for executed proposals
    ...
    constructor(
        ...
        _grantRole(bytes32(0),         _admin);
        _grantRole(GOVERNOR_ROLE,      _governor);
        _grantRole(GUARDIAN_ROLE,      _guardian);

        // Initialize security system with LockEngine
        _initSecuritySystem(_access, _time); // _access is lockEngine
    }
