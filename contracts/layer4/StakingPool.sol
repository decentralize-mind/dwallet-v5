...
contract StakingPool is ERC20, Ownable, AccessControl, ReentrancyGuard, Pausable, SecurityGated {
    ...
    uint256 public maxDeposit = 10_000_000e18; // 10M DWT max per deposit
    uint256 public lastFeeUpdate;
    uint256 public constant FEE_UPDATE_COOLDOWN = 7 days;
    ...
    event MaxDepositUpdated(uint256 newLimit);
    ...
    ) ERC20("Staked DWT", "sDWT") Ownable(_admin) SecurityGated(_securityController) {
        require(_token != address(0), "StakingPool: zero token");
        require(_admin != address(0), "StakingPool: zero admin");
        require(_governor != address(0), "StakingPool: zero governor");
        require(_securityController != address(0), "StakingPool: zero security");
        ...
        returns (uint256 shares)
    {
        ...
        require(dwtAmount <= maxDeposit, "StakingPool: exceeds max deposit");
    ...
    function setWithdrawFee(uint256 newFeeBps) external onlyOwner whenProtocolNotPaused {
        ...
        require(block.timestamp >= lastFeeUpdate + FEE_UPDATE_COOLDOWN, "StakingPool: cooldown active");
        withdrawFeeBps = newFeeBps;
        lastFeeUpdate = block.timestamp;
        emit WithdrawFeeUpdated(newFeeBps);
    }

    function setMaxDeposit(uint256 newLimit) external onlyOwner whenProtocolNotPaused {
        require(newLimit > 0, "StakingPool: zero limit");
        maxDeposit = newLimit;
        emit MaxDepositUpdated(newLimit);
    }
    ...
    /**
     * @notice Emergency withdrawal - allows users to withdraw even when paused
     * @dev Proportional withdrawal without fees during emergencies
     */
    function emergencyWithdraw() external nonReentrant {
        uint256 shares = balanceOf(msg.sender);
        require(shares > 0, "StakingPool: no shares");
        
        uint256 dwtOut = sharesToDWT(shares);
        require(dwtOut > 0, "StakingPool: zero amount");
        
        totalDWT -= dwtOut;
        _burn(msg.sender, shares);
        
        dwtToken.safeTransfer(msg.sender, dwtOut);
        emit EmergencyWithdrawal(msg.sender, dwtOut);
    }
