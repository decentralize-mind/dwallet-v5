...
export default function PriceAlertsPanel() {
  ...
  useEffect(() => {
    ...
    fetchMarketData()
      .then(data => {
        const p = {};
        data.forEach(coin => { p[coin.symbol.toUpperCase()] = coin.current_price; });
        setPrices(p);
      })
      .catch(() => {})
  ...
  const handleAddAlert = async () => {
    ...
    setAlerts(getAlerts());
    setThreshold("");
    setSuccess(`Alert set — you will be notified when ${symbol} goes ${direction} $${val.toLocaleString()}`);
  ...
  return (
    <div className="view-container">
      <div className="view-header">
        ...
        {loading && (
          <span style={{fontSize:11,color:"var(--text3)"}}>Fetching prices...</span>
        )}
      </div>
      ...
      {permStatus !== "granted" && (
        <div style={{
          ...
          <div style={{flex:1}}>
            <p style={{fontSize:12,fontWeight:700,margin:0,color:"var(--amber)"}}>
              Enable notifications to receive alerts
            </p>
            <p style={{fontSize:11,color:"var(--text3)",margin:"2px 0 0"}}>
              Toklo uses browser push notifications to alert you when prices move
            </p>
          </div>
          <button
      ...
      {permStatus === "granted" && (
        <div style={{
          ...
          <span style={{fontSize:12,color:"var(--green)",fontWeight:600}}>
            Notifications enabled — alerts will fire even when the tab is in the background
          </span>
        </div>
      )}
      ...
      <div style={{
        ...
        <div style={{display:"flex",flexDirection:"column",gap:10}}>
          <div style={{display:"flex",flexDirection:"column",gap:6}}>
            <label style={{fontSize:12,fontWeight:600,color:"var(--text2)"}}>Coin</label>
            <div style={{
              display:"grid",
              ...
              {COINS.map(c => (
                <button
                  key={c.symbol}
                  onClick={() => { setSymbol(c.symbol); setError(""); }}
                  style={{
                    padding:"8px 4px",
                    background: symbol===c.symbol ? "var(--accent)" : "var(--bg3)",
                    color: symbol===c.symbol ? "white" : "var(--text2)",
                    border: "1px solid " + (symbol===c.symbol ? "var(--accent)" : "var(--border)"),
                    borderRadius:8,cursor:"pointer",
                    fontSize:11,fontWeight:symbol===c.symbol?700:400,
          ...
          {/* Direction selector */}
          <div style={{display:"flex",flexDirection:"column",gap:6}}>
            <label style={{fontSize:12,fontWeight:600,color:"var(--text2)"}}>Alert when price goes</label>
              ...
              {[
                ...
                <button
                  ...
                  style={{
                    ...
                    background: direction===d.val ? d.color+"18" : "var(--bg3)",
                    color: direction===d.val ? d.color : "var(--text2)",
                    border: "1px solid " + (direction===d.val ? d.color+"60" : "var(--border)"),
                    borderRadius:"var(--radius-sm)",cursor:"pointer",
                    fontSize:13,fontWeight:direction===d.val?700:400,
                    fontFamily:"var(--font)",transition:"all 0.15s"
                  }}
                >
                  {d.label}
          ...
          <div style={{display:"flex",flexDirection:"column",gap:6}}>
            ...
            <div style={{position:"relative"}}>
              <span style={{
                ...
                fontSize:14,color:"var(--text3)",fontWeight:600
              }}>$</span>
              <input
                className="field"
                type="number"
          ...
          <button
            ...
            disabled={saving || !threshold.trim() || isNaN(parseFloat(threshold))}
          >
        ...
        <div style={{display:"flex",flexDirection:"column",gap:0}}>
          ...
          <div style={{display:"flex",flexDirection:"column",gap:8}}>
            {alerts.map(alert => {
              ...
              const triggered = cp
                ? (isAbove ? cp >= alert.threshold : cp <= alert.threshold)
                : false;
              return (
                    ...
                    <p style={{fontSize:12,color:"var(--text3)",margin:"3px 0 0"}}>
                      ...
                      {cp && (
                        <span style={{marginLeft:8,color:"var(--text3)"}}>
                          · Now: ${cp.toLocaleString(undefined,{minimumFractionDigits:2,maximumFractionDigits:2})}
                        </span>
                      )}
                    </p>
                  </div>
