...
export default function DWTStakingPanel() {
  ...
  useEffect(() => {
    ...
    async function fetchProtocol() {
      // Just demoing how to query the read-only view. 
      // Replace with actual provider when user connects wallet.
      const rpc = new ethers.JsonRpcProvider("https://sepolia.infura.io/v3/83c1e84032b24b0cb759b8e9fce69893"); 
  ...
  useEffect(() => {
    ...
    setStaked(s.staked||0); setReward(s.reward||0);
  }, []);

  const save = (st, rw) => {
    localStorage.setItem("dwallet_dwt_staking", JSON.stringify({staked:st,reward:rw}));
  };
  ...
  const handleStake = async () => {
    ...
    if (!amt || amt < MIN_STAKE) return notify("error", "Minimum stake is "+MIN_STAKE+" DWT");
    if (amt > dwtBal)            return notify("error", "Insufficient DWT balance");
    setLoading(true);
    await new Promise(r => setTimeout(r, 1500));
    const ns = staked + amt;
    setStaked(ns); save(ns, reward);
    setStakeInput("");
    ...
    <div className="staking-panel">
      <div className="dwt-stats-grid">
        ...
        <div className="dwt-stat">
          ...
          <p className="dwt-stat-value">{parseFloat(protocolStaked).toFixed(0)} DWT</p>
      ...
      {reward > 0 && (
        <div className="dwt-reward-banner">
          ...
          <button className="btn-primary" onClick={handleClaim} disabled={loading}>Claim</button>
        </div>
      )}

      <div className="staking-tabs">
        {["stake","unstake","info"].map(t => (
          <button key={t} className={"staking-tab"+(tab===t?" staking-tab--active":"")}
            onClick={() => setTab(t)}>{t.charAt(0).toUpperCase()+t.slice(1)}</button>
        ))}
      </div>

      {msg && (
        <p className={"field-hint"+(msg.type==="success"?" positive":msg.type==="error"?" error-msg":"")}>
          {msg.text}
        </p>
      )}
      ...
      {tab === "stake" && (
        <div className="staking-form">
          <div className="amount-input-row">
            <input className="field" type="number" placeholder={"Min "+MIN_STAKE+" DWT"}
              value={stakeInput} onChange={e => setStakeInput(e.target.value)}/>
            <button className="max-btn" onClick={() => setStakeInput(dwtBal.toFixed(0))}>MAX</button>
          </div>
          <p className="field-hint">Available: {dwtBal.toFixed(0)} DWT</p>
          {stakeInput && parseFloat(stakeInput) >= MIN_STAKE && (() => {
            const p = proj(parseFloat(stakeInput));
            return (
              <div className="dwt-projection">
                <p className="dwt-proj-title">Estimated ETH earnings at {APY}% APY</p>
                <div className="dwt-proj-row"><span>Daily</span><span className="positive">{p.daily} ETH</span></div>
                <div className="dwt-proj-row"><span>Monthly</span><span className="positive">{p.monthly} ETH</span></div>
                <div className="dwt-proj-row"><span>Yearly</span><span className="positive">{p.yearly} ETH</span></div>
              </div>
            );
          })()}
          <button className="btn-primary full-width" onClick={handleStake}
      ...
      {tab === "unstake" && (
        <div className="staking-form">
          {staked <= 0 ? (
            ...
            <>
              <div className="amount-input-row">
                <input className="field" type="number" placeholder="Amount to unstake"
                  value={unstakeIn} onChange={e => setUnstakeIn(e.target.value)}/>
                <button className="max-btn" onClick={() => setUnstakeIn(staked.toFixed(0))}>MAX</button>
              </div>
              <p className="field-hint">Staked: {staked.toFixed(0)} DWT</p>
              <button className="btn-primary full-width" onClick={handleUnstake}
      ...
      {tab === "info" && (
        <div className="dwt-info">
          <div className="dwt-info-item">
            ...
            <p className="dwt-info-text">Stake DWT to earn a share of all swap fee revenue. Rewards are paid in ETH — not more DWT — making them immediately valuable.</p>
          </div>
          <div className="dwt-info-item">
            <p className="dwt-info-label">Holder discount</p>
            <p className="dwt-info-text">Hold 1,000+ DWT to reduce your swap fee from 0.20% to 0.15% on every trade.</p>
          </div>
          <div className="dwt-info-item">
            <p className="dwt-info-label">Minimum stake</p>
            <p className="dwt-info-text">Minimum 100 DWT. No lock-up period — unstake anytime.</p>
          </div>
        </div>
      )}
    </div>
