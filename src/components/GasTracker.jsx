...
const CHAINS = [
  { id:"ethereum", name:"Ethereum",  symbol:"ETH",   icon:"⟠", color:"#6366f1" },
  { id:"polygon",  name:"Polygon",   symbol:"MATIC", icon:"⬡", color:"#8b5cf6" },
  { id:"bnb",      name:"BNB Chain", symbol:"BNB",   icon:"🔶", color:"#f59e0b" },
  { id:"arbitrum", name:"Arbitrum",  symbol:"ETH",   icon:"🔵", color:"#3b82f6" },
];
...
function getGasLevel(gwei) {
  ...
  if (g <= 10)  return { label:"Low",       color:"var(--green)", bg:"rgba(16,185,129,0.1)",  tip:"Great time to transact — fees are very low" };
  if (g <= 25)  return { label:"Normal",    color:"var(--accent)",bg:"rgba(99,102,241,0.1)",  tip:"Typical gas — go ahead and transact" };
  if (g <= 50)  return { label:"High",      color:"var(--amber)", bg:"rgba(245,158,11,0.1)",  tip:"Fees are elevated — consider waiting" };
  return              { label:"Very High",  color:"var(--red)",   bg:"rgba(239,68,68,0.1)",   tip:"Very high fees — wait for lower gas if possible" };
}
...
async function fetchGasPrice(chainId) {
  ...
  try {
    ...
    // Fallback estimate
    const fallbacks = { ethereum:"18", polygon:"60", bnb:"3", arbitrum:"0.1" };
  ...
  return (
    <div className="view-container">
        ...
        {CHAINS.map(c => (
          <button
            ...
            style={{
              ...
              border: "1px solid " + (chain===c.id ? c.color+"60" : "var(--border)"),
              color: chain===c.id ? c.color : "var(--text2)",
              fontSize:12,fontWeight: chain===c.id ? 700 : 400,
            ...
            return (
              <div key={tx.label} style={{
                ...
                <div style={{display:"flex",alignItems:"center",gap:8}}>
                  ...
                  <span style={{fontSize:12,color:"var(--text2)"}}>{tx.label}</span>
                </div>
                <span style={{
        ...
        <div style={{
          ...
          <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:12}}>
            ...
            <div style={{display:"flex",gap:12,fontSize:10,color:"var(--text3)"}}>
              <span>Low: {Math.min(...hist.map(h=>h.gwei)).toFixed(1)} Gwei</span>
              <span>High: {Math.max(...hist.map(h=>h.gwei)).toFixed(1)} Gwei</span>
            </div>
          </div>
          ...
          <div style={{
            ...
            gap:3,height:60,paddingBottom:4
          }}>
            {hist.map((h, i) => {
      ...
      <div style={{
        ...
        <p style={{fontSize:12,fontWeight:700,margin:"0 0 8px",color:"var(--accent)"}}>
          💡 Best times to transact
        </p>
        <div style={{display:"flex",flexDirection:"column",gap:6}}>
          {[
            ["🟢","Weekends (Sat–Sun)",             "30–50% lower gas on average"],
            ["🟢","Weekday nights (UTC 00:00–08:00)","US and EU markets asleep"],
            ["🔴","Weekday afternoons (UTC 14:00–20:00)","US + EU markets overlap — highest fees"],
            ["🟡","Monday mornings",                 "Fees spike as markets open"],
          ].map(([dot, time, note]) => (
            <div key={time} style={{
              display:"flex",alignItems:"flex-start",gap:8
            }}>
              <span style={{fontSize:10,marginTop:1,flexShrink:0}}>{dot}</span>
              <div>
                <span style={{fontSize:12,fontWeight:600,color:"var(--text2)"}}>{time}</span>
                <span style={{fontSize:11,color:"var(--text3)",marginLeft:6}}>{note}</span>
              </div>
            </div>
          ))}
        </div>
