...
export default function MainWallet() {
  ...
  const renderMain = () => {
    ...
    switch (activeTab) {
      case "dashboard": return <Dashboard onSend={() => setModal("send")} onReceive={() => setModal("receive")} onSwap={() => setActiveTab("defi")}/>;
      case "defi":      return <DefiView/>;
      case "history":   return <TransactionHistory/>;
      case "nfts":      return <NFTsView/>;
      case "dapps":     return <DAppsView/>;
      case "settings":  return <SettingsView onNavigate={setSubView}/>;
      default:          return <Dashboard onSend={() => setModal("send")} onReceive={() => setModal("receive")} onSwap={() => setActiveTab("defi")}/>;
    }
  };
    ...
    <div className="wallet-shell">
      <header className="topbar">
        <div className="topbar-left">
          {subView ? (
            <button className="icon-btn" onClick={() => setSubView(null)} style={{fontSize:18}}>←</button>
          ) : (
        ...
        <div className="topbar-right">
          ...
          <button className="icon-btn" onClick={lockWallet} title="Lock">🔒</button>
        </div>
      </header>
      ...
      <nav className="bottom-nav">
        {NAV_ITEMS.map(item => (
          <button
            key={item.id}
            className={"nav-item " + (activeTab === item.id && !subView ? "nav-item--active" : "")}
