...
export default function LendingPanel() {
  ...
  const usdValue = (
    parseFloat(amount || 0) * (TOKEN_PRICES[asset.symbol] || 1)
  ).toFixed(2)
  ...
  const handleAction = async () => {
    ...
    try {
      ...
      if (import.meta.env.VITE_INFURA_KEY) {
        if (action === 'supply')
          tx = await aaveSupply({ asset: asset.symbol, amount, privateKey: pk })
        if (action === 'withdraw')
          tx = await aaveWithdraw({
            asset: asset.symbol,
            amount,
            privateKey: pk,
          })
        if (action === 'borrow')
          tx = await aaveBorrow({ asset: asset.symbol, amount, privateKey: pk })
        if (action === 'repay')
          tx = await aaveRepay({ asset: asset.symbol, amount, privateKey: pk })
        setTxHash(tx.hash)
      } else {
        setTxHash(
          '0x' +
  ...
  const hfColor = !accountData
    ...
    : parseFloat(accountData.healthFactor) >= 2
      ? 'positive'
      : parseFloat(accountData.healthFactor) >= 1.2
        ? 'warn'
        : 'negative'
      ...
      {accountData && (
        <div className="aave-account-summary">
          <div className="aave-stat">
            ...
            <span className="aave-stat-value">
          ...
          <div className="aave-stat">
            ...
            <span className="aave-stat-value">
              ${accountData.availableBorrowUSD}
            </span>
          </div>
          <div className="aave-stat">
            <span className="aave-stat-label">Health</span>
            <span className={`aave-stat-value ${hfColor}`}>
      ...
      <div className="defi-mode-tabs">
        {MODES.map(m => (
          <button
            ...
            onClick={() => {
              setMode(m)
              setAction(m === 'Supply' ? 'supply' : 'borrow')
              setAmount('')
            }}
          >
            {m}
        ...
        <>
          ...
          <div className="defi-action-tabs">
            {mode === 'Supply'
              ? ['supply', 'withdraw'].map(a => (
                  <button
                    key={a}
                    className={`defi-action-tab ${action === a ? 'active' : ''}`}
                    onClick={() => setAction(a)}
                  >
            ...
            {AAVE_ASSETS.map(a => (
              ...
              >
                ...
                <div className="aave-asset-right">
                  <span className="aave-asset-supply positive">
                    Supply {a.supplyAPY}%
                  </span>
                  <span className="aave-asset-borrow warn">
                    Borrow {a.borrowAPY}%
                  </span>
                </div>
          ...
          <div className="defi-input-group">
            ...
            <p className="field-hint">
              ≈ ${usdValue} &nbsp;·&nbsp; Wallet: {balance.toFixed(4)}{' '}
              {asset.symbol}
            </p>
          </div>
          ...
          {mode === 'Borrow' && (
            <div className="aave-hf-warning">
              ⚠️ Keep health factor above 1.0 to avoid liquidation. Recommended:
              stay above 1.5.
            </div>
          )}
          ...
          >
            {loading
              ? 'Processing...'
              : `${action.charAt(0).toUpperCase() + action.slice(1)} ${amount || '0'} ${asset.symbol}`}
          </button>
        </>
      )}
      ...
      {step === 'success' && (
        <div className="defi-success">
          ...
          <p className="success-sub">
            {amount} {asset.symbol} via Aave V3
          </p>
          {mode === 'Supply' && (
            <p className="success-note">
              Earning {asset.supplyAPY}% APY on your deposit
            </p>
          )}
          {mode === 'Borrow' && (
            <p className="success-note">
              Repay at {asset.borrowAPY}% APY variable rate
            </p>
          )}
