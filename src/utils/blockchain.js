...
export async function estimateGas(chainId = "ethereum") {
  try {
    ...
    return { gwei: "20", ethCost: "0.000420", gasPrice: ethers.parseUnits("20", "gwei") };
  }
}

// ── Send transaction ──────────────────────────────────────────────────────────
export async function sendNative(to, amountEth, privateKey, chainId = "ethereum") {
...
export async function sendERC20(tokenAddress, to, amount, decimals, privateKey, chainId = "ethereum") {
  const signer   = getSigner(privateKey, chainId);
...
export async function fetchTxHistory(address, chainId = "ethereum") {
  ...
  try {
    ...
    return data.result.slice(0, 50).map(tx => ({
      ...
      token:     chainId === "ethereum" ? "ETH" : chainId === "bnb" ? "BNB" : "MATIC",
      type:      tx.from.toLowerCase() === address.toLowerCase() ? "send" : "receive",
      status:    tx.isError === "0" ? "confirmed" : "failed",
      timestamp: parseInt(tx.timeStamp) * 1000,
      gasUsed:   parseFloat(ethers.formatEther(
...
export async function fetchTokenTxHistory(address, chainId = "ethereum") {
  ...
  try {
    ...
    return data.result.slice(0, 30).map(tx => ({
      ...
      from:      tx.from,
      to:        tx.to,
      value:     parseFloat(ethers.formatUnits(tx.value, parseInt(tx.tokenDecimal))),
