...
export function setProposalHandler(fn)    { onProposal = fn; }
export function setRequestHandler(fn)     { onRequest  = fn; }
...
export async function pairWithDapp(uri) {
  ...
  setTimeout(() => {
    if (onProposal) onProposal({
      id: Date.now(),
      params: {
        proposer: { metadata: { name:"dApp", description:"", url:"", icons:[] } },
        requiredNamespaces: {
          eip155: { chains:["eip155:1"], methods:["eth_sendTransaction","personal_sign"], events:[] },
        },
      },
    });
  }, 800);
...
export async function approveSession(proposal, accounts) {
  const session = { topic:`session_${Date.now()}`, peer:proposal.params.proposer, accounts };
  activeSessions[session.topic] = session;
  return session;
}

export async function rejectSession()                            {}
export async function disconnectSession(topic)                   { delete activeSessions[topic]; }
