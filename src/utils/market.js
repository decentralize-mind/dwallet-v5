...
const FALLBACK={
  BTC:{price:67000,change:2.1},ETH:{price:3200,change:1.8},
  SOL:{price:180,change:3.2},BNB:{price:420,change:0.9},
  XRP:{price:0.62,change:-0.5},ADA:{price:0.45,change:1.2},
  AVAX:{price:38,change:2.4},DOT:{price:7.8,change:-1.1},
  MATIC:{price:0.85,change:1.5},LINK:{price:14.2,change:2.8},
  DOGE:{price:0.12,change:4.2},ATOM:{price:9.4,change:-0.8},
  NEAR:{price:5.8,change:3.1},ARB:{price:1.12,change:1.9},
  OP:{price:2.34,change:2.2},AAVE:{price:92,change:1.4},
  UNI:{price:8.5,change:1.3},USDC:{price:1.00,change:0.0},
};
...
export async function fetchMarketData(){
  ...
  try{
    ...
    marketCache=result;lastFetch=now;return result;
  }catch{
    if(marketCache)return marketCache;
    return MARKET_COINS.map(coin=>({...coin,
      price:FALLBACK[coin.symbol]?.price??0,
      change24h:FALLBACK[coin.symbol]?.change??0,
      marketCap:0,volume24h:0,rank:99,
    }));
  }
}
